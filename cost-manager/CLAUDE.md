...
- 実処理時間（active time）はコスト集計とは独立の transcript 走査（`scan_activity`）で算出し、
  `iter_usage` / `Accumulator` / `collect_dedup_rows` / `aggregate` の dedup ロジックには触れないこと。
- 期間終端（`end_display`）はレポートターン除外後の最終アクティビティ時刻に補正されること
  （`--until` 明示時を除く）。
