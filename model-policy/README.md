...
メインループを高単価モデル（Fable 5）で運用しつつ、**サブエージェント（Agent ツール / Workflow の agent()）が誤って Fable で起動すること**を Claude Code のハーネスレベルで防ぐ仕組み。Fable を「計画・設計・統括」専任にし、実作業は安価な Opus サブエージェントへ委譲させてコストを下げる。手動サフィックス「サブエージェントは opus を使用して」を不要にする。
...
ハード強制は「**fable 禁止・fork 禁止・model 未指定→opus 書き換え**」の 3 点に絞る。opus/sonnet/haiku など fable 以外は素通しする（プラグイン/組み込みエージェントの安価モデル指定を壊さないため）。「全サブエージェント Opus 固定」というコスト方針そのものは、hook ではなく行動規範層（CLAUDE.md）で担保する。
...
| 2 | `~/.claude/CLAUDE.md` への追記 | fable=統括専任・作業は opus サブエージェントへ委譲・agent() は model 明示・fork 禁止 | 規範（システムコンテキスト常駐で compact 後も残る） |
...
**何が強制で何が規範か**: 「fable がサブエージェントに渡らない」ことは層1/1b が**ハード強制**する（compact 後も常に効く）。「全部を opus にする（sonnet/haiku を選ばない）」は層2 の**行動規範**であって hook は強制しない。
...
- **サブエージェントの model は必ず `"opus"` を明示する**（現行方針: 全サブエージェント opus 固定）。省略は fable 継承となるため禁止。fable 指定も禁止。
- **Workflow / ultracode**: script 内の全 `agent()` 呼び出しに `model: 'opus'` を明示する。
...
- 上記は PreToolUse hook でも強制される。拒否されたら理由に従い model を修正して再実行すること。
```
...
### 7-1. 全 Opus 固定 ↔ Sonnet 使い分けへの切替
今は「全サブエージェント Opus 固定」。Sonnet 使い分けへ移す手順は 2 点だけ:
1. CLAUDE.md の該当節を「実装・調査・デバッグ=opus / 探索・grep・機械的作業・要約=sonnet」の振り分け基準に差し替える。
2. hook・ポリシーファイルは**変更不要**（`allowed` に `sonnet` が既に入っており、hook は fable 以外を素通しするため）。
