...
| 4 | `scripts/model_policy_reminder_hook.sh`（UserPromptSubmit） | 緩和中／恒久 model=fable／fable 例外の失効48時間前のときだけ注入（平常時は無出力=トークンゼロ） | 可視化 |
...
- `fable_exempt_until`: 例外の有効期限（epoch 秒）。`null`/過去なら例外は**期限切れ＝deny 動作**（fable の課金条件変化で例外が黙って実費を生む事故を防ぐ安全側フェイル）。延長は `model_policy.sh exempt [日数]`。
...
- **Claude Code バージョン**: 動作確認済みバージョン → **2.1.202**（2026-07-07 検証。実機で確認済み: model 未指定→opus 書き換え〔サブエージェントのモデルID自己申告で `claude-opus-4-8[1m]` を確認〕、fable 指定→deny、fork→deny、relax 中の fable 通過→reset で deny 復帰、Workflow の model 語ゼロ→deny / model 値 fable→deny（2026-07-13 再検証。fable 文字列を含むが model 明示の script は通過）、fable 例外（`fable_exempt_subagent_types`＋TTL、2026-07-18 追加・単体テストで exempt 素通し/期限切れ deny/リスト外 deny の3系を確認））。本システムは以下の文書化仕様に依存する:
...
model_policy.sh exempt [日数]   # fable 例外の期限を now+日数（既定14・上限90）。exempt off で即解除
...
Opus メイン運用で「判断の要所だけ Fable に相談する」ための例外機構。`policy.json` に登録し、TTL で期限を切る:
```bash
...
bash ~/.claude/skills/model-policy/scripts/model_policy.sh exempt 14   # 期限 = now + 14日
```
- 判定は agent hook の fork チェック直後: `subagent_type` が**完全一致**かつ `now < fable_exempt_until` のときだけ素通し（fable deny と空 model→opus 書き換えの両方を回避）。fork は引き続き無条件 deny。
- **期限切れは rewrite ではなく明示 deny**（黙って opus に化けて「advisor のつもりが opus だった」品質事故を防ぐ）。理由文に延長手順が入る。
- 期限が残り48時間を切ると、層4（reminder hook）が毎プロンプトに失効予告を注入する。
