...
CLI は必ず**絶対パス**で Bash から叩く（`settings.json` の permissions.allow に登録済み。`$HOME` 変数だと permissions のリテラル一致から外れて許可プロンプトが出る）。以下の `/Users/<YOU>` は自分のホームパスに読み替える（`echo $HOME` で確認）。install.sh を使えば自動置換される（README §3-4 参照）。
