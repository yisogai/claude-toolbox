...
> **状態（2026-08-01）: 凍結（Opus メイン運用時代のアーカイブ）**
> 作者環境はメインループを Fable 5 に移行し、本ハーネスの hooks（完了ゲート・検証台帳・
> nudge）と fable-advisor の運用を停止した。きっかけは2つ: (1) Claude 5 世代の公式
> ガイダンス（旧モデル向けの過剰な指示・検証強制はかえって品質を下げる）(2) 完了ゲートの
> 実運用監査で、4日間のブロック26件中88%が誤爆（scratchpad の一時スクリプト誤判定・
> RSpec 系のパターン未対応が主因）と確認されたこと。
> Opus をメインループに使う環境向けの参考実装としてそのまま残しており、install.sh /
> UNINSTALL.md は引き続き動作する。導入する場合は下記に加えて誤爆特性（テストコマンドが
> 台帳パターンに載らないと完了時にブロックされる）を理解した上で使うこと。
